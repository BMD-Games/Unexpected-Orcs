...
public class Generator {
    ...
    public static int[][] generateCave(int w, int h, int iterations, float chance) {
        ...
        for (int i = 0; i < w; i ++) {
            for (int j = 0; j < h; j ++) {
                if (Util.random(1) < chance) tiles[i][j] = Tiles.WALL;
                else {
                    tiles[i][j] = Tiles.FLOOR;
                }
            }
        }
    ...
    public static int numNeighbours(int[][] tiles, int x, int y, int dist) {
        ...
        for (int i = -dist; i <= dist; i ++) {
            for (int j = -dist; j <= dist; j ++) {
                ...
                try {
                    if (tiles[x + i][y + j] == Tiles.WALL) num ++; //if the tile is a wall
                }
                catch(Exception e) {
                }
            }
...
//------DUNGEON GENERATION---------
    ...
    public static int[][] generateWindyDungeon(int w, int h, int roomAttempts, int minSize, int maxSize, float straightChance, float loopChance) {
        ...
        for (int i = edgeSize; i < w-edgeSize; i ++) {
            for (int j = edgeSize; j < h-edgeSize; j ++) {
                if (numNeighbours(tiles, i, j, 1) == 8 && tiles[i][j] == Tiles.WALL) { //seed point found
                    int x, y, dir = 0;
        ...
        while (connected.size() < regionCount) {
            int n = (int)Util.random(connectors.size());
            ...
            if (c == 2 || c == 3) {
                ...
                tiles[connector[0]][connector[1]] = Tiles.FLOOR;
            }
        }

        //add Util.random connections to make dungeon more interesting
        for (int i = 0; i < connectors.size(); i ++) {
            if (Util.random(1) < loopChance) {
                tiles[connectors.get(i)[0]][connectors.get(i)[1]] = Tiles.FLOOR;
            }
        }
        ...
        for (int i = edgeSize; i < w-edgeSize; i ++) {
            for (int j = edgeSize; j < h-edgeSize; j ++) {
                if (tiles[i][j] == Tiles.WALL) continue;
                int x = i;
                int y = j;
    ...
    public static ArrayList<int[]> placeRooms(int w, int h, int roomAttempts, int minSize, int maxSize) {
        ...
        for (int i = 0; i < roomAttempts; i ++) {
            ...
            xl = (int)Math.floor(Util.random(minSize, maxSize)); //room width
            yl = (int)Math.floor(Util.random(minSize, maxSize)); //room height
            x = (int)Math.floor(Util.random(edgeSize, w - xl - edgeSize)); //room x pos - avoid edges
            y = (int)Math.floor(Util.random(edgeSize, h - yl - edgeSize)); //toom y pos - avoid edges
            int[] room = {x, y, xl, yl};
            boolean hit = false;
    ...
    public static int getEndDirection(int[][] tiles, int i, int j) {
        if (numNeighboursSimple(tiles, i, j) < 3 || tiles[i][j] == Tiles.WALL) return -1; //not dead end
        //returns the direction of the corridor from a dead end
        if (tiles[i][j-1] == Tiles.FLOOR) return 0; //up
        if (tiles[i][j+1] == Tiles.FLOOR) return 2; //down
        if (tiles[i-1][j] == Tiles.FLOOR) return 3; //left
        if (tiles[i+1][j] == Tiles.FLOOR) return 1; //right
        return -1;
    }
    ...
    public static boolean validTile(int[][] tiles, int x, int y, int dx, int dy) {
        ...
        for (int i = -1; i <= 1; i ++) {
            for (int j = -1; j <= 1; j ++) {
                ...
                try {
                    if (tiles[x + i][y + j] >= Tiles.FLOOR) return false; //if the tile is a floor
                }
                catch(Exception e) {
                    return false;
                }
    ...
    public static void addConnector(ArrayList<int[]> connectors, int[][] tiles, int[][] region, int i, int j) {
        if (tiles[i][j] != Tiles.WALL) return;
        ...
        if ((t >= Tiles.FLOOR && b >= Tiles.FLOOR) && tr != br) { // >= Tiles.FLOOR is non-solid block
            connectors.add(new int[] {i, j, tr, br});
        } else if ((l >= Tiles.FLOOR && r >= Tiles.FLOOR) && lr != rr) {
    ...
    public static boolean isBorder(int[][]tiles, int i, int j) {
        ...
        try {
            border = ((numNeighbours(tiles, i, j, 1) < 8 && tiles[i][j] == Tiles.WALL));
        }
        catch(Exception e) {
        }
        return (!edge && border);
  ...
  /* ---Pseudo---
   ...
   - get a Util.random room from the graph
   ...
   */
        ...
        float dir = Util.random((float)Math.PI * 2); //Util.random direction for the dungeon to branch in.
        ...
        while (placedRooms.size() < maxRooms) {
            int startPos = (int)Util.random(placedRooms.size()); //get a Util.random room;
            int sx = placedRooms.get(startPos).x;
            int sy = placedRooms.get(startPos).y;
            int newPos = placedRooms.size();
            boolean hit = true;
            Room room = new Room(rooms[(int)Util.random(rooms.length)]);
            ...
            while (hit) {
                hit = false;
                float ang = Util.random(dir - spread, dir + spread);
                float r = Util.random(minRadius, maxRadius) + Math.max(Math.max(placedRooms.get(startPos).w, placedRooms.get(startPos).h), Math.max(room.w, room.h));
                room.x = (int)(sx + Math.cos(ang) * r);
                room.y = (int)(sy + Math.sin(ang) * r);
    ...
    public static int[][] connectRooms(int[][] tiles, Room r1, Room r2) {
        ...
        int[] dir = {dx, 0};
        int[] dir2 = {0, dy};
        if (Util.fastAbs(dx) < Util.fastAbs(dy)) { //do large axis first
            dir[0] = 0;
            dir[1] = dy;
            dir2[0] = dx;
            dir2[1] = 0;
    ...
    public static int[][] finishingPass(int[][] tiles, TileSet tileset) {
        ...
        for (int i = 0; i < w; i ++) {
            for (int j = 0; j < h; j ++) {
                ...
                } else if (tiles[i][j] == Tiles.FLOOR) {
                    //use some Util.random shit to add flavour to dungeons
                    if (tileset.extras.size() > 0 && Util.random(1) < 0.1) {
                        newTiles[i][j] = tileset.extras.get((int)Util.random(tileset.extras.size()));
