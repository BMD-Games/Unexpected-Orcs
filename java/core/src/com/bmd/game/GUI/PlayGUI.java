...
public class PlayGUI implements Screen {
    ...
    private ImageButton pause = new ImageButton(new SpriteDrawable(Sprites.guiSprite("PAUSE", Tiles.TILE_SIZE, Tiles.TILE_SIZE)));

    public PlayGUI(UnexpectedOrcs g) {
        game = g;
        ...
        pause.setPosition(Util.width - 2 * Tiles.TILE_SIZE, Util.height - 2 * Tiles.TILE_SIZE);
        pause.setSize(Tiles.TILE_SIZE, Tiles.TILE_SIZE);
        pause.addListener(new ClickListener() {
            @Override public void touchUp(InputEvent e, float x, float y, int p, int b) { pauseClicked(); }
        });
